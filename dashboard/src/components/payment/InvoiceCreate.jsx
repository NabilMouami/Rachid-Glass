...
const FactureCreate = () => {
  // Use current date with time if not selected
  ...
  // TVA state
  const [tvaRate, setTvaRate] = useState(20); // Default to 20% (standard Moroccan rate)
  ...
  // Ensure date is set to current date/time on mount if not provided
  useEffect(() => {
    if (!startDate) {
    ...
    if (!selectedOption) {
      const updatedItems = items.map((item) => {
        if (item.id === itemId) {
          ...
          // Recalculate total using rounded dimensions
  ...
  // Calculate subtotal (before discount and TVA) using rounded dimensions
  ...
  useEffect(() => {
    const remaining = total - advancementPrice;
    setRemainingAmount(remaining > 0 ? remaining : 0);
  }, [total, advancementPrice]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const calculatedRemaining = Math.max(0, total - advancementPrice);
      ...
      const invoiceData = {
        ...
        // Send advancement as a single value
        ...
        // User who created/validated the invoice
      ...
      console.log("📦 Sending Facture data to backend:", invoiceData);

      // Send to backend
      const response = await axios.post(
        `${config_url}/api/factures`,
        invoiceData,
      );

      if (response.data.success) {
        const facture = response.data.facture || response.data.bon;
        const totalAdvancements = response.data.totalAdvancements || 0;

        console.log("✅ Success data:", response.data);

        if (facture && facture.id) {
          setCreatedInvoiceId(facture.id);
        }

        // Show appropriate success message based on alerts
        if (hasAlerts) {
          topTost("Facture créée avec des prix hors fourchette!", "warning");
        } else {
        ...
        <div style="text-align:left;font-size:14px">
          <p><strong>Numéro :</strong> ${facture.num_facture || "N/A"}</p>
          <p><strong>Date :</strong> ${new Date(startDate).toLocaleString("fr-FR")}</p>
          <p><strong>Montant HT :</strong> ${calculatedTotalHT.toFixed(2)} DH</p>
          <p><strong>TVA (${tvaRate}%) :</strong> ${calculatedTvaAmount.toFixed(2)} DH</p>
          <p><strong>Montant TTC :</strong> ${calculatedTotalTTC.toFixed(2)} DH</p>
          <p><strong>Acomptes versés :</strong> ${totalAdvancements} DH</p>
          <p><strong>Montant restant :</strong> ${(calculatedTotalTTC - advancementPrice).toFixed(2)} DH</p>
          <p><strong>Statut :</strong> ${facture.status || "N/A"}</p>
      ...
      if (error.response) {
        ...
        if (error.response.data.errors) {
          ...
          topTost("Erreur lors de la création de la facture", "error");
        }
        ...
        <div className="card invoice-container">
          ...
          <div className="card-body p-0">
            <div className="px-4 pt-4">
              ...
              {/* Status, Payment Type, TVA, and Advancement */}
            ...
            <div className="px-4 pb-4">
              <div className="form-group">
                ...
                <textarea
                  ...
                  className="form-control"
                  id="InvoiceNote"
                  placeholder="It was a pleasure working with you and your team. We hope you will keep us in mind for future projects. Thank You!"
                  value={invoiceNote}
